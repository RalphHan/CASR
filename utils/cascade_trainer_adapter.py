...
class CascadeSeq2SeqTrainerAdapter(Seq2SeqAdapterTrainer):
    def __init__(
            self,
            evaluator,
    ...
    def train_all(self):
        ...
        if self.args.do_restart and self.is_world_process_zero():
            os.makedirs(os.path.join(self.origin_output_dir,'init'), exist_ok=True)
            self.model.save_all_adapters(os.path.join(self.origin_output_dir,'init'))
        torch.distributed.barrier()
        for cascade_step in range(self.args.max_cascade_steps):
            if self.args.do_restart and cascade_step>0:
                for adp in ['adapter1', 'adapter2']:
                    self.model.load_adapter(os.path.join(self.origin_output_dir,'init',adp))
