...
def draw(prediction):
    ...
    for i,c in enumerate(prediction):
        if c==0 or c>9:
            cv2.rectangle(img,(10+i%9*26, 10+i//9*26),(36+i%9*26, 36+i//9*26),(220,220,220),-1)
        if c>0:
            txt=str((c-1)%9+1)
...
if __name__=='__main__':
    ...
    os.makedirs(os.path.join(path,'pic2'),exist_ok=True)
    ...
    for casstep in range(5):
        ...
        for order in tqdm(slice):
            os.makedirs(os.path.join(path, 'pic2', str(order)),exist_ok=True)
            gt=dataset[order]
            ...
            if casstep==0:
                img_gt=draw(tgt)
                cv2.imwrite(os.path.join(path, 'pic2', str(order),'gt.png'),img_gt)
            pd=pred[order]-3
            pd[(src==0)&(pd+9==tgt)]+=9
            img_pd = draw(pd)
            cv2.imwrite(os.path.join(path, 'pic2', str(order), f'{casstep}.png'), img_pd)
