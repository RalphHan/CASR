...
def main() -> None:
    ...
    with torch.no_grad():
        for input,pred in zip(tqdm(test_dataset),predictions):
            ...
            lm_logits=model(**input).logits
            labels=torch.tensor(pred[1:]).cuda()
            labels[labels==0]=-100
            loss = loss_fct(lm_logits.view(-1, lm_logits.size(-1)), labels).item()
            losses.append(loss)
    losses=np.float32(losses)
    torch.save(losses,os.environ["TO"]+'.pb')
