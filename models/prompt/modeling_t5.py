...
class T5Stack(T5PreTrainedModel):
    ...
    def forward(
            ...
            past_prompt_encoder_key='encoder_prompt',
    ):
        # Model parallel
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            encoder_prefix = past_prompt[i][past_prompt_encoder_key] if past_prompt else None  # TODO: Chen
        ...
        if encoder_outputs is None:
            ...
            if mode=='sepenc':
                if last_predictions is not None:
                    assert attention_mask is not None
                    last_prediction_mask=(last_predictions != self.config.pad_token_id).to(attention_mask)
                    encoder_outputs_lp = self.encoder(
                        ...
                        return_dict=return_dict,
                        past_prompt=past_prompt,  # TODO: Chen
                        past_prompt_encoder_key='encoder_prompt_lp',
                    )
                    encoder_outputs = BaseModelOutput(
