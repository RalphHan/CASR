...
class Model(PushToHubFriendlyModel):
    def __init__(self, args):
        ...
        for name in ['prefix', 'prefix_enc', 'prefix_dec']:
            setattr(self, name,
                    VersionPrefix(args.max_cascade_steps if name != 'prefix_enc' else 1, self.args.model.freeze_prefix,
    ...
    def update_version(self):
        ...
        if self.prefix_enc.version == -1:
            self.prefix_enc.update_version()
        else:
            for param in self.prefix_enc.parameters():
                param.requires_grad = False
    ...
    def forward(self,
        ...
        if last_predictions is not None:
            shift_right = self.pretrain_model._shift_right(labels)
            decoder_input_ids = torch.cat(
                (last_predictions, self.middle_prompt.repeat(last_predictions.size(0), 1), shift_right), dim=-1)
            labels = torch.cat(
                (torch.full((bsz, decoder_input_ids.size(1) - labels.size(1)), -100).to(input_ids), labels), dim=-1)
    ...
    def generate(self,
        ...
        if last_predictions is not None:
            decoder_input_ids = torch.cat((last_predictions, self.middle_prompt.repeat(last_predictions.size(0), 1)),
                                          dim=-1)
