...
class ProgressiveDataset(Dataset):
    ...
    def __init__(self, args, training_args, tokenizer, seq2seq_dataset,vocab_mask):
        self.args = args
    ...
    def __getitem__(self, index):
        ...
        tokenized_inferred=[x for x in origin_tokenized_inferred[:-1] if self.vocab_mask[self.cascade_step,x]==0.0]+origin_tokenized_inferred[-1:]
        ...
        if hasattr(self, 'last_predictions'):
            ...
            middle=[x for x in origin_tokenized_inferred[:-1] if
                    self.vocab_mask[self.cascade_step - 1, x] == 0.0]
